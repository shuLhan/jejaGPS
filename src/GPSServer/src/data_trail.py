...
try:
	...
	q ="""
 select dt_retrieved, latitude, longitude from trail where id_gps = %s order by dt_retrieved desc;
"""
	data	=(id_gps,)
	...
	for r in cur:
		if i > 0:
			...
			i = 1
		o+="{ dt_retrieved:'%s', latitude:'%s', longitude:'%s'}" % (r[0].isoformat(' '), r[1], r[2])
